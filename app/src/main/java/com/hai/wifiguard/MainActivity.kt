...
@Composable
private fun AuditApp(viewModel: AuditViewModel = viewModel()) {
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            state.result?.let { result ->
                ...
                item {
                    NoticeCard(
                        "الفحص يقتصر على الشبكة المتصل بها الجهاز والبوابة المحلية، ولا ينفذ تخمين كلمات مرور أو تعطيل أجهزة أو استغلال ثغرات.",
                        warning = false,
                    )
                }
            } ?: item {
...
private fun HeroCard(
    ...
    val status = result?.let(::securityStatusText) ?: "جاهز للفحص"
        ...
        Column(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(
                        "تحليل نوع الحماية وWPS والراوتر وكلمة المرور محليًا.",
            ...
            Button(
                ...
                if (scanning) {
                    ...
                    Text(if (result == null) "ابدأ الفحص العملي" else "أعد الفحص")
                }
            }
        }
    }
...
private fun ConnectionCard(result: WifiAuditResult) {
    SectionCard(
        ...
        InfoRow(
            "WPS",
            when (result.wps) {
                WpsState.DETECTED -> "ظاهر — يفضّل تعطيله"
                WpsState.NOT_ADVERTISED -> "غير ظاهر في بيانات المسح"
        ...
        if (result.permissionLimited) {
            ...
            Text(
                "بعض معلومات Wi‑Fi قد تكون محجوبة بسبب صلاحيات Android.",
...
private fun RouterCard(result: WifiAuditResult) {
    SectionCard(
        ...
        InfoRow(
            "واجهات الإدارة المحلية",
            if (result.openAdminPorts.isEmpty()) "لم تظهر على المنافذ الشائعة" else result.openAdminPorts.joinToString("، "),
        )
        Text(
            "الفحص هنا يختبر فقط 80 / 443 / 8080 / 8443 على Default Gateway ولا يفحص أجهزة الشبكة الأخرى.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
    }
...
private fun PasswordCard(ssid: String) {
    ...
    SectionCard(
        title = "اختبار كلمة المرور محليًا",
        icon = { Icon(Icons.Default.Security, contentDescription = null) },
    ) {
        Text(
            "اكتب كلمة مرور شبكتك إذا أردت تقييم قوتها. لا تُحفظ ولا تُرسل لأي خادم.",
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
...
private fun EmptyState() {
    Card(
        ...
        Column(
            ...
            Spacer(Modifier.height(8.dp))
            Text("اتصل بشبكة Wi‑Fi التي تملكها ثم اضغط «ابدأ الفحص العملي».")
        }
    }
}
